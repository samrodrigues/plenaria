  ...
  <div class="mt-3">
    <div class="mt-1">
      <Combobox as="div" v-model="member">
        <ComboboxLabel class="block text-sm font-medium text-gray-700">Digite seu nome para pesquisar</ComboboxLabel>
        <div class="relative mt-1">
          <ComboboxInput class="w-full rounded-md border border-gray-300 bg-white py-2 pl-3 pr-10 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 sm:text-sm" @change="query = $event.target.value" :display-value="(person) => person?.name" />
          ...
          <ComboboxOptions v-if="filteredPeople.length > 0" class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            <ComboboxOption v-for="person in filteredPeople" :key="person.id" :value="person" as="template" v-slot="{ active, selected }">
              <li :class="['relative cursor-default select-none py-2 pl-8 pr-4', active ? 'bg-emerald-600 text-white' : 'text-gray-900']">
            <span :class="['block truncate', selected && 'font-semibold']">
              {{ person.name }}
            </span>

                <span v-if="selected" :class="['absolute inset-y-0 left-0 flex items-center pl-1.5', active ? 'text-white' : 'text-emerald-600']">
    ...
    <div class="mt-1 flex gap-1">
      <div class="flex-grow-1">
        ...
        <div class="mt-1">
          <input type="number" v-model="dob.day" min="1" max="31" name="day" id="day" class="shadow-sm focus:ring-emerald-500 focus:border-emerald-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="DD" />
        </div>
      </div>
      <div class="flex-grow-1">
        <div>
      ...
      <div class="year-container">
        <label for="year" class="block text-sm font-medium text-gray-700">Ano (AAAA)</label>
        <div class="mt-1">
          <input v-model="dob.year" min="1900" max="2100" type="number" name="year" id="year" class="shadow-sm focus:ring-emerald-500 focus:border-emerald-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="AAAA" />
        </div>
      </div>
    </div>
    ...
    filteredPeople() {
      return this.query === ''
          ...
          : members.filter((person) => {
            return person.name.toLowerCase().startsWith(this.query.toLowerCase())
          })
    }
  },
  methods: {
